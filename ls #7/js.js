...
const map = {
   ...
   * @param {{x: food.x, y: food.y}} foodPoint Точка еды.
...
const food = {
   ...
   * @returns {{x: food.x, y: food.y}} Координаты еды.
   */
  getCoordinates() {
    return {
      x: this.x,
...
const game = {
  ...
  renderScore() {
    ...
    if (!this.isGameWon()) {
      scoreElement.innerHTML = `${this.snake.getBody().length - 1} / ${this.config.getWinFoodCount()}`;
    } else {
      scoreElement.innerHTML = 'Вы победили!';
    }
